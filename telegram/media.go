...
func (j *downloadJob) requestTimeout(limit int, attempt int) time.Duration {
	if j.requestTimeoutOverride > 0 {
		return j.requestTimeoutOverride
	}
	base := 45 * time.Second
	switch {
...
func (c *Client) DownloadChunk(media any, start int, end int, chunkSize int, opts ...any) ([]byte, string, error) {
